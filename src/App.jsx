  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', height: '100vh' }}>
      <Sidebar
        ...
        setActiveIndex={(i) => { stopTour(); setActiveIndex(i); }}
        onTourStart={startTour}
        onPrev={() => setActiveIndex((activeIndex - 1 + PLACES.length) % PLACES.length)}
        onNext={() => setActiveIndex((activeIndex + 1) % PLACES.length)}
        onAnimateRoute={() => tourRef.current.animateRoute()}
      ...
      />
      <MapView activeIndex={activeIndex} setActiveIndex={setActiveIndex} tourControl={tourRef} />
    </div>
  )
}
